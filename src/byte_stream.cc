...
ByteStream::ByteStream( uint64_t capacity ) : 
  capacity_( capacity ), buffer_(), close_(0), bytes_buffered_(0), 
  bytes_pushed_(0), bytes_popped_(0), first_string_left_size(0), error_(0)  {}
...
{
  // Your code here.
  uint64_t allowed_capacity = available_capacity();
  uint64_t to_push_size = min(allowed_capacity, data.size()); 
  if(to_push_size == 0) return;
  data.resize(to_push_size);
  buffer_.push(std::move(data));
  if(buffer_.size() == 1)
    first_string_left_size = to_push_size;
  bytes_pushed_ += to_push_size;
  bytes_buffered_ += to_push_size;
  return;
}
...
{
  // Your code here.
  if(bytes_buffered_ == 0 || buffer_.empty()){
    return string_view{};
  }
